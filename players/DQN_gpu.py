...
class DQN_GPU():
    ...
    def not_learning(self, game):
        input_data = xp.array([game.parse()], dtype=xp.float32)
    ...
    def learn(self, s, a, r, fs, terminal):
        ...
        """
        max_qs = np.max(self.target_model(xp.array(list(fs), dtype=xp.float32), dropout=self.DQN).data, axis=1)

        y = self.model(xp.array(list(s), dtype=xp.float32), dropout=self.DQN)
        t = copy.deepcopy(y)
